...
class Agent:
    ...
    def Solve(self, problem):
        ...
        if problem.problemType == '3x3':
            ...
            cyclic_comparison = self.cyclic_pattern(prob_fig, prob_array, ans_fig, ans_array)
            ...
            if self.compare_images(prob_array['A'],prob_array['B']) == 0:
            ...
            elif self.compare_images(prob_array['A'],prob_array['E']) == 0 or self.getFigureDarkpixels(prob_array['A'],prob_array['E'])==0:
                print('Cyclic pattern')
                m = self.image_combo(prob_fig, prob_array)
                if m[2] < self.compare_images(prob_array['A'], prob_array['E']) or m[0] < self.getFigureDarkpixels(prob_array['A'], prob_array['E']) :
                    print('answer is a combo')
                    answer = self.ans_combo(ans_fig,ans_array, prob_fig, prob_array, m[3])
                    return answer                        
                else:
                    for i in range(1,9):
            ...
            elif cyclic_comparison[4] == cyclic_comparison[0][3]:
                    print("another cyclic pattern")
                    sec_min = self.find_second_min(ans_array,prob_array['E'])
                    result = sec_min[1]
                    for i in range(1,9):
                        if self.getFigureDarkpixels(ans_array[str(i)],prob_array['E']) == min(cyclic_comparison[3]):
                            print(min(cyclic_comparison[3]))
                            if min(cyclic_comparison[3]) == 0:
                                answer = result
                                print(answer)
                                return answer
                            else:
                ...
                print(m[0])
                print(m[1])
                if (m[2] < self.compare_images(prob_array['B'], prob_array['C'])) or (m[0] < self.getFigureDarkpixels(prob_array['B'],prob_array['C'])):
                    print('Combo')
                    answer = self.ans_combo(ans_fig,ans_array, prob_fig, prob_array, m[3])
                    return answer
                else:
    ...
    def find_min(self, ans_array, array2):
        ...
        answer_compare1 = [self.getFigureDarkpixels(ans_array[str(x)],array2) for x in range(1,9)]
    ...
    def cyclic_pattern(self, prob_fig, prob_array, ans_fig, ans_array):
        ...
        aindex = np.argmin(aDict)
        bindex = np.argmin(bDict)
        cindex = np.argmin(cDict)


        Elist = [self.getFigureDarkpixels(ans_array[str(x)],prob_array['E']) for x in range(1,9)]
        
        results = [aDict,bDict,cDict, Elist, aindex,bindex,cindex]
        return results


    def find_nearest_neighbor(self, answer_array, prob_array):
        answer_compare = [self.getFigureDarkpixels(ans_array[str(x+ 1)],prob_array['D']) for x in range(len(ans_array))]
    ...
    def ans_combo(self, ans_fig, ans_array, prob_fig,prob_array,min_index):
        ...
        if min_index == 0:
            ...
            HIpixel = [self.getFigureDarkpixels(HI[x], prob_array['G']) for x in range(0,8)]
            ...
            GIpixel = [self.getFigureDarkpixels(GI[x], prob_array['H']) for x in range(0,8)]
    ...
    def image_difference(self, prob_fig, prob_array, ans_fig, ans_array):
        ...
        arrayList = [diffarrayAB, diffarrayGH, diffarrayAD, diffarrayBE, diffarrayDG, diffarrayEH]
        return arrayList
